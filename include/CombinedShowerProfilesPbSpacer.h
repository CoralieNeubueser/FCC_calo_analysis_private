#ifndef COMBINEDSHOWERPROFILESPbSpacer_H
#define COMBINEDSHOWERPROFILESPbSpacer_H
...
class CombinedShowerProfilesPbSpacer: public BaseAnalysis {

 public:
  CombinedShowerProfilesPbSpacer(double aEnergy);
  ~CombinedShowerProfilesPbSpacer();
 ...
 private:
  ...
  // ATTENTION Lambda, instead of X0 !!!
  const double X0_ecal = 294.0; // in mm for ECAL, eta=0
  const double X0_hcal = 200.7; // in mm for HCAL, eta=0
  const double X0_zeroOffset = .3;  // X0 before the ECAL starts
  const double X0_offset = 2.2;  // X0 before the HCAL starts
...
#endif /* COMBINEDSHOWERPROFILESPbSpacer_H */
